...
```{r}
#step 2 for interaction
# Load necessary libraries
library(lme4)
library(lmerTest)

# Load the dataset
DataSet1 <- read.csv(file = 'C:\\Users\\timothy\\OneDrive\\Documents\\ISU\\food_project\\data\\data_for_test\\tea_coffee.csv')

# Define required columns for tea and coffee
required_columns_tea <- c("rsfMRI_Component7", "rsfMRI_Component11", "rsfMRI_Component14", 
...
# Create complete datasets for tea and coffee
DataSet1_complete_tea <- DataSet1[complete.cases(DataSet1[, required_columns_tea]), ]
DataSet1_complete_coffee <- DataSet1[complete.cases(DataSet1[, required_columns_coffee]), ]

# Define dependent variables and predictors
dependent_vars <- c("rsfMRI_Component7", "rsfMRI_Component11", "rsfMRI_Component14", "rsfMRI_Component21", "rsfMRI_Component12")
predictors <- c("StandTea_mean", "FilterCoffee_mean")

# Function to fit the model and extract coefficients for multiple interaction terms
fit_and_extract <- function(dep_var, predictor, interaction_terms, data, factor_label, factor_value) {
  # Create the interaction part of the formula dynamically
  interaction_formula <- paste(interaction_terms, collapse = " + ")
  
  formula <- as.formula(paste(dep_var, "~ Assessment_0 + APOE_all + T40_Gcar + family_history_AD + 
                              Age_Baseline + Sex + SmokingY08 + AlcoholY08 + 
                              BMI_Y08andY12andY14_mean + Townsend +", predictor, "+", interaction_formula, "+ 
                              (1 | Assessment_0)"))
  
  model <- lmer(formula, data = data, REML = FALSE)
  summary_model <- summary(model)
  
  # Ensure the predictor exists in the coefficients
  if (predictor %in% rownames(summary_model$coefficients)) {
    predictor_coef <- summary_model$coefficients[predictor, ]
    
    return(list(
      Dependent = dep_var,
      Predictor = predictor,
      Estimate = predictor_coef["Estimate"],
      StdError = predictor_coef["Std. Error"],
      TValue = predictor_coef["t value"],
      PValue = predictor_coef["Pr(>|t|)"],
      Factor_Label = factor_label,
      Factor_Value = factor_value
    ))
  } else {
    # Return NA if the predictor isn't found
    return(list(
      Dependent = dep_var,
      Predictor = predictor,
      Estimate = NA,
...
# Run the loop to test each combination and store results for selected interactions
results <- list()
index <- 1
...
for (dep_var in dependent_vars) {
  for (predictor in predictors) {
    ...
    # Test both predictors for APOE_all and family_history_AD
    if (predictor == "FilterCoffee_mean") {
      interaction_terms <- c("FilterCoffee_mean * APOE_all", "FilterCoffee_mean * family_history_AD")
      
      # Test for both APOE_all = 0 and APOE_all = 1
      for (APOE_all_val in c(0, 1)) {
        data_subset <- subset(DataSet1_complete_coffee, APOE_all == APOE_all_val)
        result <- fit_and_extract(dep_var, predictor, interaction_terms, data_subset, "APOE_all", APOE_all_val)
        results[[index]] <- result
        index <- index + 1
      }
      
      # Test for both family_history_AD = 0 and family_history_AD = 1
      for (family_history_val in c(0, 1)) {
        data_subset <- subset(DataSet1_complete_coffee, family_history_AD == family_history_val)
        result <- fit_and_extract(dep_var, predictor, interaction_terms, data_subset, "family_history_AD", family_history_val)
        results[[index]] <- result
        index <- index + 1
      }
      
    } else if (predictor == "StandTea_mean") {
      interaction_terms <- c("StandTea_mean * APOE_all", "StandTea_mean * family_history_AD")
      
      # Test for both APOE_all = 0 and APOE_all = 1
      for (APOE_all_val in c(0, 1)) {
        data_subset <- subset(DataSet1_complete_tea, APOE_all == APOE_all_val)
        result <- fit_and_extract(dep_var, predictor, interaction_terms, data_subset, "APOE_all", APOE_all_val)
        results[[index]] <- result
        index <- index + 1
      }
      ...
      # Test for both family_history_AD = 0 and family_history_AD = 1
      for (family_history_val in c(0, 1)) {
        data_subset <- subset(DataSet1_complete_tea, family_history_AD == family_history_val)
        result <- fit_and_extract(dep_var, predictor, interaction_terms, data_subset, "family_history_AD", family_history_val)
        results[[index]] <- result
        index <- index + 1
      }
...
write.csv(results_df, "C:\\Users\\timothy\\OneDrive\\Documents\\ISU\\food_project\\output\\tea_coffee\\interactions_report.csv", na='', row.names=FALSE)
